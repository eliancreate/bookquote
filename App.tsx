...
export default function App() {
  ...
  return <ThemeContext.Provider value={value}><SafeAreaProvider><Main theme={theme} palettes={palettes} gradients={gradients[theme]} cardBorder={cardBorder} setCardBorder={setCardBorder} setGradientColor={(name, index, color, persist) => setGradientColor(theme, name, index, color, persist)} setColor={setColor} setTheme={next => { setTheme(next); storage.save('linekeep_theme', next); }} /></SafeAreaProvider></ThemeContext.Provider>;
}

function Main({ theme, palettes, gradients, cardBorder, setCardBorder, setTheme, setColor, setGradientColor }: { theme: ThemeName; palettes: ThemePalettes; gradients: ActionGradients; cardBorder: boolean; setCardBorder: (next: boolean) => void; setTheme: (theme: ThemeName) => void; setColor: (theme: ThemeName, role: keyof ThemeColors, color: string, persist?: boolean) => void; setGradientColor: (name: keyof ActionGradients, index: number, color: string, persist?: boolean) => void }) {
  const { colors: C, styles: s } = useTheme();
  ...
  return <SafeAreaView edges={['top', 'left', 'right']} style={s.screen}>
    ...
    <View style={s.header}>
      ...
      <Pressable accessibilityRole="button" accessibilityLabel="設定" accessibilityState={{ selected: tab === 'settings' }} onPress={() => { if (tab !== 'settings') haptic.select(); setTab('settings'); setSearch(''); setSearchOpen(false); scrollAnchor.current = 0; setNavVisible(true); }} style={s.headerIcon}><Icon name={tab === 'settings' ? 'cog' : 'cog-outline'} size={23} color={C.ink} /></Pressable>
    </View>
    ...
    <View style={s.content}>
      ...
      {tab === 'settings' && <Settings theme={theme} palettes={palettes} gradients={gradients} cardBorder={cardBorder} setCardBorder={setCardBorder} onScroll={onContentScroll} setTheme={setTheme} setColor={setColor} setGradientColor={setGradientColor}
...
function Settings({ theme, palettes, gradients, notesCount, cardBorder, setCardBorder, onScroll, onOpenNotes, setTheme, setColor, setGradientColor, onExport, onImport, onRestore, onClear }: { theme: ThemeName; palettes: ThemePalettes; gradients: ActionGradients; notesCount: number; cardBorder: boolean; setCardBorder: (next: boolean) => void; onScroll: (y: number) => void; onOpenNotes: () => void; setTheme: (v: ThemeName) => void; setColor: (theme: ThemeName, role: keyof ThemeColors, color: string, persist?: boolean) => void; setGradientColor: (name: keyof ActionGradients, index: number, color: string, persist?: boolean) => void; onExport: () => void; onImport: () => void; onRestore: () => void; onClear: () => void }) {
  const { colors: C, styles: s } = useTheme();
  ...
  return <ScrollView showsVerticalScrollIndicator={false} scrollEventThrottle={16} onScroll={event => onScroll(event.nativeEvent.contentOffset.y)} contentContainerStyle={s.list}>
    ...
    <Section title="顯示樣式"><View style={[s.filterRow, s.softSurface]}><Text style={s.filterTitle}>卡片邊線</Text><Switch value={cardBorder} onValueChange={v => { haptic.select(); setCardBorder(v); }} trackColor={{ false: C.line, true: C.accent }} thumbColor={C.panel} /></View></Section>
...
function createStyles(C: ThemeColors, cardBorder = false) {
  ...
  quoteShell: { borderRadius: 22, overflow: 'hidden', backgroundColor: C.panel }, swipeActions: { position: 'absolute', right: 0, top: 0, bottom: 0, width: 112, flexDirection: 'row' }, swipeAction: { flex: 1, alignItems: 'center', justifyContent: 'center' }, editAction: { backgroundColor: C.line }, deleteAction: { backgroundColor: C.panel }, quoteCard: { minHeight: 190, padding: 20, borderRadius: 22, backgroundColor: C.panel, ...(cardBorder && { borderWidth: 1, borderColor: C.line }) }, quoteCardTap: { flex: 1 }, quoteBody: { flexGrow: 1, justifyContent: 'center', paddingVertical: 14 }, quote: { color: C.ink, lineHeight: 31, fontWeight: '500', textAlign: 'center' }, quoteFooter: { alignItems: 'flex-end' }, bookName: { color: C.ink, fontSize: 14, fontWeight: '700', textAlign: 'right' }, author: { color: C.muted, fontSize: 13, textAlign: 'right', marginTop: 3 },
