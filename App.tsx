...
function Sheet({ visible, tall = false, title, onClose, children }: { visible: boolean; tall?: boolean; title: string; onClose: () => void; children: React.ReactNode }) { const { colors: C, styles: s, t } = useTheme(); return <Modal visible={visible} transparent statusBarTranslucent navigationBarTranslucent animationType="slide" onRequestClose={onClose}><SafeAreaProvider><SafeAreaView edges={['top', 'right', 'left']} style={s.overlay}><KeyboardAvoidingView behavior={Platform.OS === 'ios' ? tall ? 'height' : 'padding' : undefined} style={s.sheetAvoider}><SafeAreaView edges={['bottom']} style={[s.sheet, tall && s.sheetTall]}><View style={s.titleRow}><Text style={s.title}>{title}</Text><Pressable accessibilityLabel={t('關閉')} onPress={onClose} style={s.iconButton}><Icon name="close" size={20} color={C.muted} /></Pressable></View><ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={s.form}>{children}</ScrollView></SafeAreaView></KeyboardAvoidingView></SafeAreaView></SafeAreaProvider></Modal>; }
