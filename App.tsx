...
export default function App() {
  ...
  const value = useMemo(() => ({ colors, styles: createStyles(colors, cardBorder && theme === 'sunlight') }), [colors, cardBorder, theme]);
...
function Settings({ theme, palettes, gradients, notesCount, cardBorder, setCardBorder, onScroll, onOpenNotes, setTheme, setColor, setGradientColor, onExport, onImport, onRestore, onClear }: { theme: ThemeName; palettes: ThemePalettes; gradients: ActionGradients; notesCount: number; cardBorder: boolean; setCardBorder: (next: boolean) => void; onScroll: (y: number) => void; onOpenNotes: () => void; setTheme: (v: ThemeName) => void; setColor: (theme: ThemeName, role: keyof ThemeColors, color: string, persist?: boolean) => void; setGradientColor: (name: keyof ActionGradients, index: number, color: string, persist?: boolean) => void; onExport: () => void; onImport: () => void; onRestore: () => void; onClear: () => void }) {
  ...
  return <ScrollView showsVerticalScrollIndicator={false} scrollEventThrottle={16} onScroll={event => onScroll(event.nativeEvent.contentOffset.y)} contentContainerStyle={s.list}>
    ...
    {theme === 'sunlight' && <Section title="顯示樣式"><View style={[s.filterRow, s.softSurface]}><Text style={s.filterTitle}>卡片邊線</Text><Switch value={cardBorder} onValueChange={v => { haptic.select(); setCardBorder(v); }} trackColor={{ false: C.line, true: C.accent }} thumbColor={C.panel} /></View></Section>}
