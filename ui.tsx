...
function Sheet({ visible, tall = false, title, onClose, children }: { visible: boolean; tall?: boolean; title: string; onClose: () => void; children: React.ReactNode }) {
  const { colors: C, styles: s, t } = useTheme();
  const dragY = useRef(new Animated.Value(0)).current;
  ...
  const pan = useMemo(() => PanResponder.create({
    ...
    onPanResponderRelease: (_, g) => {
      if (g.dy > 90 || g.vy > 0.8) Animated.timing(dragY, { toValue: 700, duration: 180, useNativeDriver: true }).start(() => { dragY.setValue(0); closeRef.current(); });
  ...
  return <Modal visible={visible} transparent statusBarTranslucent navigationBarTranslucent animationType="slide" onRequestClose={onClose}><SafeAreaProvider><SafeAreaView edges={['top', 'right', 'left']} style={s.overlay}><KeyboardAvoidingView behavior={Platform.OS === 'ios' ? tall ? 'height' : 'padding' : undefined} style={s.sheetAvoider}><AnimatedSafeAreaView edges={['bottom']} style={[s.sheet, tall && s.sheetTall, { transform: [{ translateY: dragY }] }]}><View {...pan.panHandlers}><View style={s.sheetGrabArea}><View style={s.sheetGrabber} /></View><View style={s.titleRow}><Text style={s.title}>{title}</Text><Pressable accessibilityLabel={t('關閉')} onPress={onClose} style={s.iconButton}><Icon name="close" size={20} color={C.muted} /></Pressable></View></View><ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={s.form}>{children}</ScrollView></AnimatedSafeAreaView></KeyboardAvoidingView></SafeAreaView></SafeAreaProvider></Modal>;
}
...
function createStyles(C: ThemeColors) {
  ...
  screen: { flex: 1, backgroundColor: C.bg }, statusBarGuard: { position: 'absolute', zIndex: 30, top: 0, left: 0, right: 0, backgroundColor: C.bg }, texture: { ...StyleSheet.absoluteFillObject, opacity: 0.14, pointerEvents: 'none' }, flex: { flex: 1 }, loading: { margin: 'auto', color: C.muted },
  header: { zIndex: 20, backgroundColor: C.bg, paddingHorizontal: 20, paddingTop: 14, paddingBottom: 12, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  ...
  drawButtonShell: { minWidth: 116, minHeight: 44, borderRadius: 14, shadowOpacity: .16, shadowRadius: 11, shadowOffset: { width: 0, height: 0 }, elevation: 3 }, drawButton: { minHeight: 44, borderRadius: 14, overflow: 'hidden', paddingHorizontal: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 7 }, drawShimmer: { position: 'absolute', top: -22, bottom: -22, width: 28, backgroundColor: '#FFFFFFCC' }, drawButtonText: { fontSize: 15, fontWeight: '600' }, disabled: { opacity: .4 }, homeTools: { zIndex: 20, minHeight: 52, backgroundColor: C.bg, paddingHorizontal: 20, paddingTop: 12, paddingBottom: 8, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: 8 }, homeToolsRight: { flexDirection: 'row', alignItems: 'center', gap: 8 }, headerIcon: { width: 44, height: 44, alignItems: 'center', justifyContent: 'center' }, iconButton: { ...softSurface, width: 44, height: 44, borderRadius: 22, backgroundColor: C.panel, alignItems: 'center', justifyContent: 'center' }, iconButtonActive: { backgroundColor: C.line }, filterBadge: { position: 'absolute', top: -3, right: -3, minWidth: 18, height: 18, borderRadius: 9, paddingHorizontal: 4, overflow: 'hidden', backgroundColor: C.accent, color: C.bg, textAlign: 'center', fontSize: 12, fontWeight: '800' },
  ...
  overlay: { flex: 1, backgroundColor: 'transparent' }, sheetAvoider: { flex: 1, justifyContent: 'flex-end' }, sheet: { maxHeight: '92%', backgroundColor: C.bg, borderTopLeftRadius: 30, borderTopRightRadius: 30, paddingHorizontal: 22, paddingTop: 6, paddingBottom: 22, shadowColor: '#07111C', shadowOpacity: 0.18, shadowRadius: 20, shadowOffset: { width: 0, height: -6 }, elevation: 24 }, sheetTall: { height: '91%', maxHeight: '91%' }, sheetGrabArea: { minHeight: 30, alignItems: 'center', justifyContent: 'center' }, sheetGrabber: { width: 38, height: 4, borderRadius: 2, backgroundColor: C.line }, form: { gap: 15, paddingBottom: 20 }, input: { minHeight: 48, backgroundColor: C.panel, color: C.ink, borderRadius: 14, paddingHorizontal: 14, fontSize: 16, borderWidth: 1, borderColor: C.line }, textarea: { minHeight: 120, paddingTop: 14 }, filterList: { gap: 8 }, filterRow: { minHeight: 52, paddingHorizontal: 14, paddingVertical: 8, borderRadius: 14, backgroundColor: C.panel, flexDirection: 'row', alignItems: 'center', gap: 10 }, filterTitle: { flex: 1, color: C.ink, fontSize: 16, fontWeight: '600' }, filterCount: { color: C.muted, fontSize: 13, fontWeight: '700' },
  });
}
...
export type { ActionGradients, Excerpt, Lang, Note, Tab, ThemeName };
